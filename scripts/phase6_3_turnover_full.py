...
def yearly_performance_tables(daily_all: pd.DataFrame, costs: list[float]) -> pd.DataFrame:
    rows = []
    x = daily_all.copy()
    x["calendar_year"] = pd.to_datetime(x["date"], errors="coerce").dt.year
    for (h, variant, year), g in x.groupby(["horizon_days", "rebalance_variant", "calendar_year"], sort=True):
        r = perf_stats(g, "portfolio_abnormal_return")
        r.update({"horizon_days": h, "model": "lightgbm", "rebalance_variant": variant, "calendar_year": int(year), "cost_bps": 0.0, "net_or_gross": "gross", "period": str(int(year))})
        rows.append(r)
        for bps in costs:
            s = str(bps).replace(".", "p")
            col = f"net_abnormal_return_cost_{s}bps"
            if col in g:
                r = perf_stats(g, col)
                r.update({"horizon_days": h, "model": "lightgbm", "rebalance_variant": variant, "calendar_year": int(year), "cost_bps": bps, "net_or_gross": "net", "period": str(int(year))})
                rows.append(r)
    return pd.DataFrame(rows)
...
def make_figures(out_dir: Path, perf: pd.DataFrame, validation: pd.DataFrame, daily_all: pd.DataFrame) -> list[dict[str, str]]:
    ...
    if not p10.empty:
        ...
        plt.title("Phase 6.3 full low-turnover backtest: net abnormal Sharpe")
        plt.tight_layout()
        path = fig_dir / "phase6_3_net_sharpe_10bps.png"
    ...
    if not validation.empty:
        ...
        plt.tight_layout()
        path = fig_dir / "phase6_3_turnover_by_variant.png"
        plt.savefig(path, dpi=180)
        plt.close()
    ...
    if not daily_all.empty:
        ...
        plt.title("10 bps net cumulative abnormal return, full-test variants")
        plt.legend(fontsize=8)
        plt.tight_layout()
        path = fig_dir / "phase6_3_cumulative_10bps.png"
        plt.savefig(path, dpi=180)
        plt.close()
...
def render_report(out_path: Path, quality: dict[str, Any], perf: pd.DataFrame, validation: pd.DataFrame, figures: list[dict[str, str]], env: dict[str, Any]) -> None:
    ...
    doc = f"""<!doctype html>
<html lang='en'><head><meta charset='utf-8'><title>Phase 6.3 Low-Turnover Full Backtest</title>
<style>
...
</style></head><body><header><h1>Phase 6.3 Low-Turnover Full Backtest</h1><p class='subtitle'>Local-only full 2022-2024 backtest of the low-turnover LightGBM portfolio variants selected after the Phase 6.2 pilot. No WRDS queries; protected positions remain local.</p></header><main>
...
def main() -> int:
    ...
    protected = root / "data" / "processed" / "backtests" / "phase6_3_turnover_full"
    ...
    print("=" * 80)
    print("Phase 6.3 low-turnover full backtest")
    print(f"UTC: {utc_now()}")
    ...
    for h in horizons:
        for variant in variants:
            try:
                print(f"[FULL] variant={variant}, horizon={h}d")
                ...
                print(f"[FULL] {variant} h={h}: daily_rows={len(daily):,}, avg_turnover={val['avg_daily_turnover']:.4f}, avg_positions={val['avg_daily_positions']:.1f}")
            except Exception as exc:
                print(f"[WARN] failed variant={variant}, horizon={h}: {exc}")
                errors.append({"horizon_days": h, "rebalance_variant": variant, "error": repr(exc)})

    if not daily_frames:
        raise RuntimeError("No Phase 6.3 strategies completed")
    ...
    yearly_perf = yearly_performance_tables(daily_all, costs)

    daily_all.to_csv(out_dir / "phase6_3_daily_portfolio_returns.csv", index=False)
    signal_diag.to_csv(out_dir / "phase6_3_signal_date_diagnostics.csv", index=False)
    validation.to_csv(out_dir / "phase6_3_validation_diagnostics.csv", index=False)
    perf.to_csv(out_dir / "phase6_3_performance_summary.csv", index=False)
    yearly_perf.to_csv(out_dir / "phase6_3_yearly_performance_summary.csv", index=False)
    if errors:
        pd.DataFrame(errors).to_csv(out_dir / "phase6_3_errors.csv", index=False)
    ...
    if p10.empty:
        ...
        yp = yearly_perf[yearly_perf["return_col"].eq("net_abnormal_return_cost_10p0bps")].copy()
        if not yp.empty:
            yr = (
                yp.groupby(["horizon_days", "rebalance_variant"], as_index=False)
                .agg(
                    positive_sharpe_years=("ann_sharpe_naive", lambda x: int((pd.to_numeric(x, errors="coerce") > 0).sum())),
                    min_year_sharpe=("ann_sharpe_naive", lambda x: float(pd.to_numeric(x, errors="coerce").min())),
                    positive_return_years=("cumulative_return", lambda x: int((pd.to_numeric(x, errors="coerce") > 0).sum())),
                    min_year_cumulative=("cumulative_return", lambda x: float(pd.to_numeric(x, errors="coerce").min())),
                )
            )
            joined = joined.merge(yr, on=["horizon_days", "rebalance_variant"], how="left")
        else:
            joined["positive_sharpe_years"] = 0
            joined["positive_return_years"] = 0
        # Prefer robust all-year positive variants, then net Sharpe and cumulative return.
        joined["candidate_robust_score"] = (
            joined.get("positive_sharpe_years", 0).fillna(0) * 1000
            + joined.get("positive_return_years", 0).fillna(0) * 500
            + joined["ann_sharpe_naive"].fillna(-999)
        )
        best = joined.sort_values(["candidate_robust_score", "ann_sharpe_naive", "cumulative_return"], ascending=[False, False, False]).iloc[0]
    ...
    quality = {
        ...
        "note": "Local-only full turnover backtest using Phase 5.1 predictions and Phase 4.1 return panel; no WRDS query.",
    }

    with (out_dir / "phase6_3_quality_summary.json").open("w") as f:
    ...
    render_report(out_dir / "phase6_3_turnover_full_report.html", quality, perf, validation, figures, env)

    summary_lines = [
        "# Phase 6.3 low-turnover full backtest summary",
        "",
