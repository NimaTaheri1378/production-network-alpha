...
def fit_lightgbm(X_train: pd.DataFrame, y_train: np.ndarray, X_val: pd.DataFrame, y_val: np.ndarray, threads: int) -> Any:
    ...
    # Conservative pilot parameters: quick, deterministic, low overfit risk.
    model = LGBMRegressor(
...
def run_horizon(
    ...
    for split_name, split_df, y, pred in [("train", train, y_train, train_pred_lgbm), ("validation", val, y_val, val_pred_lgbm), ("test", test, y_test, test_pred_lgbm)]:
        s, ic, sp, de = summarize_prediction("lightgbm_pilot", horizon, split_name, y, pred, split_df["signal_date"])
        summaries.append(s); ic_frames.append(ic); spread_frames.append(sp); decile_frames.append(de)
    ...
    pred_cache.to_parquet(protected_dir / f"phase5_0_predictions_h{horizon}d.parquet", index=False)
...
def make_figures(out_dir: Path, summary: pd.DataFrame, deciles: pd.DataFrame, spreads: pd.DataFrame, importances: pd.DataFrame) -> list[dict[str, str]]:
    ...
    if not test.empty:
        ...
        plt.title("Phase 5.0 pilot forecast skill by horizon")
        plt.legend()
        plt.tight_layout()
        p = fig_dir / "phase5_0_test_rank_ic_by_horizon.png"
        ...
        plt.title("Phase 5.0 pilot decile spread by horizon")
        plt.legend()
        plt.tight_layout()
        p = fig_dir / "phase5_0_test_decile_spread_by_horizon.png"
        plt.savefig(p, dpi=180)
        plt.close()
        made.append({"figure": "test_decile_spread_by_horizon", "path": str(p)})

    if not deciles.empty:
        main = deciles[(deciles["split"].eq("test")) & (deciles["model"].eq("lightgbm_pilot")) & (deciles["horizon_days"].eq(5))].copy()
        if main.empty:
            main = deciles[(deciles["split"].eq("test")) & (deciles["model"].eq("lightgbm_pilot"))].copy()
        if not main.empty:
            plt.figure(figsize=(10.5, 5.8))
            ...
            plt.title("LightGBM pilot test decile monotonicity")
            plt.legend()
            plt.tight_layout()
            p = fig_dir / "phase5_0_lightgbm_decile_monotonicity.png"
            plt.savefig(p, dpi=180)
            plt.close()
            made.append({"figure": "lightgbm_decile_monotonicity", "path": str(p)})

    if not spreads.empty:
        main = spreads[(spreads["split"].eq("test")) & (spreads["model"].eq("lightgbm_pilot"))].copy()
        if not main.empty:
            ...
            plt.title("LightGBM pilot test cumulative decile spread")
            plt.legend()
            plt.tight_layout()
            p = fig_dir / "phase5_0_lightgbm_cumulative_test_spread.png"
            plt.savefig(p, dpi=180)
            plt.close()
    ...
    if not importances.empty:
        ...
        plt.title("Top LightGBM pilot features")
        plt.tight_layout()
        p = fig_dir / "phase5_0_top_lgbm_features.png"
        plt.savefig(p, dpi=180)
        plt.close()
...
def render_html(out_path: Path, quality: dict[str, Any], summary: pd.DataFrame, deciles: pd.DataFrame, importances: pd.DataFrame, figures: list[dict[str, str]], env: dict[str, Any]) -> None:
    ...
    doc = f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>Phase 5.0 Modeling Pilot</title>
<style>
...
</style></head><body><header><h1>Phase 5.0 Modeling Pilot</h1><p class="subtitle">Local-only modeling pilot using the Phase 4.2 model-ready matrix. It compares a raw spillover score, a transparent ridge/local-projection-style benchmark, and a LightGBM challenger under the fixed 2015–2019 / 2020–2021 / 2022–2024 walk-forward split.</p></header><main>
...
def main() -> int:
    ...
    parser.add_argument("--row-cap-per-year", type=int, default=60000)
    ...
    protected_dir = project_root / "data" / "processed" / "model_runs" / "phase5_0_modeling_pilot"
    ...
    print("================================================================================")
    print("Phase 5.0 modeling pilot")
    print(f"UTC: {utc_now()}")
    print(f"Project root: {project_root}")
    print(f"Output dir: {out_dir}")
    print(f"Protected local model-run dir: {protected_dir}")
    print(f"Horizons: {args.horizons}")
    print(f"Row cap per year per horizon: {args.row_cap_per_year}")
    print("================================================================================")
    ...
    summary.to_csv(out_dir / "phase5_0_model_summary.csv", index=False)
    if not ic.empty:
        ic.to_csv(out_dir / "phase5_0_daily_rank_ic.csv", index=False)
    if not spreads.empty:
        spreads.to_csv(out_dir / "phase5_0_daily_decile_spreads.csv", index=False)
    if not deciles.empty:
        deciles.to_csv(out_dir / "phase5_0_decile_monotonicity.csv", index=False)
    importances.to_csv(out_dir / "phase5_0_lgbm_feature_importances.csv", index=False)
    ...
    lgbm_test = all_test[all_test["model"].eq("lightgbm_pilot")]
    validation_checks = {
    ...
    quality = {
        ...
        "note": "Pilot uses row caps and the pure-spillover model-ready files. If diagnostics look good, Phase 5.1 should scale once with all rows/all horizons.",
    }
    with (out_dir / "phase5_0_quality_summary.json").open("w") as f:
        json.dump(quality, f, indent=2, default=str)

    figures = make_figures(out_dir, summary, deciles, spreads, importances)
    render_html(out_dir / "phase5_0_modeling_pilot_report.html", quality, summary, deciles, importances, figures, env)
    ...
    pd.DataFrame(protected_inventory).to_csv(out_dir / "protected_local_phase5_0_inventory.csv", index=False)

    lines = [
        "# Phase 5.0 modeling pilot summary",
        "",
        ...
        f"- Row cap per year/horizon: {args.row_cap_per_year:,}",
        ...
        f"- Report: {out_dir / 'phase5_0_modeling_pilot_report.html'}",
        "",
        "Data policy: protected prediction/model-run Parquet files remain local and are not included in the upload bundle.",
    ]
    (out_dir / "PHASE5_0_SUMMARY.md").write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
